...
##		    ('subcommand', 'subcommand'), "\n", ('flash', 'flash'))
    ...
    def data(self, msg):
        ...
        l = len(ochr)
        t = [128 & c for c in ochr]
        if 128 in t:
            # Found inverse video characters
            ochr = list(map(chr, [~128 & c for c in ochr]))
...
##  		self.Status['relief'] = 'flat'
    ...
    def queryCommand(self, msg, cmd, post=None, pre=None):
        ...
        def f(msg, self=self, cmd=cmd, pre=pre, post=post):
            ...
            if pre:
                self.ioq.Send(pre)
            self.ioq.Send(cmd % ((msg,)*cmd.count('%s')))
            if post:
                self.ioq.Send(post)
    ...
    def redraw(self, total=0):
        ...
        for i in [x for x in empDb.updateDB.values() if x]:
            i.clear()
...
def InitFileHandler():
    ...
    if not hasattr(_tk, 'createfilehandler') or _tk.createfilehandler is None:
