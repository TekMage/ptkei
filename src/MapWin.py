...
# Designation-character → hexName overrides for player-owned sectors.
# Sectors not listed here fall back to ownedSector / nomobSector.
_DES_HEX_NAMES = {
    # Terrain
    '^': 'mountainSector',
    # Agriculture
    ...
    'w': 'warehouseSector',
    # Military / defense
    'f': 'militarySector', 'd': 'militarySector',
    '*': 'militarySector', '+': 'militarySector',
    # Industrial / production (tech, research, bank, light industry, etc.)
    't': 'industrialSector', 'r': 'industrialSector', 'b': 'industrialSector',
    'l': 'industrialSector', 'j': 'industrialSector', 'e': 'industrialSector',
    'n': 'industrialSector', 'i': 'industrialSector', 'u': 'industrialSector',
    'p': 'industrialSector', '%': 'industrialSector', 'k': 'industrialSector',
    'm': 'industrialSector', 'g': 'industrialSector', 'o': 'industrialSector',
    's': 'industrialSector', 'h': 'industrialSector',
}
...
class mapSubWin:
    ...
    def adjustSector(self, ratio):
        ...
        self.gridsize = [ratio * g for g in self.gridsize]
        # activating/deactivating combat mode
        if (self.gridsize[0]>self.combatModeStartSize[0] and
            self.gridsize[1]>self.combatModeStartSize[1]):
                self.combatmode=1
...
##  	    self.redraw(1)
    ...
    def setOrigin(self, loc):
        ...
        import itertools
        for i in itertools.chain(range(int(y), int(scrHeight)+1, int(self.gridsize[1])),
                                 range(int(y), -1, -int(self.gridsize[1]))):
            self.Map.addtag_enclosed('move_b',
                                     x-self.gridsize[0],
            ...
            if not total:
                ...
                for i in group:
                    if "SECTOR" in self.Map.gettags(i):
                        self.Map.delete(i)
            if own == CN_OWNED:
                des_hex = _DES_HEX_NAMES.get(des)
                if des_hex:
                    hexName = des_hex
                else:
                    mob = j.get('mob')
...
#		    print "#" + str(id), type,
    ...
    def displayUnit(self, x,y, number, scale, owner, id, type, tagName):
        ...
        l = len(coords)//2
        base = (x, y)*l
        scale = (self.gridsize[0], self.gridsize[1])*l
        coords = [b + s*c for b, s, c in zip(base, scale, coords)]
...
##  	    print "Item", name, item
        ...
        l = len(coords)//2
        base = (x, y)*l
        scale = (self.gridsize[0], self.gridsize[1]*2.0/3.0)*l
        coords = [b + s*c for b, s, c in zip(base, scale, coords)]
        d = {}
        d.update(item[2])
        d.update(kw)
        # Draw it
        return item[0](*tuple(coords), **d)
...
class MoveMode:
    def __init__(self, mapClass, commodity, source, quantity=None, dest=None):
        ...
        self.lblVar.set("Right-click map to add/move waypoints. Double-right-click to remove.")
        self.AddSpot(source)
        if dest is not None:
            self.AddSpot(dest)
