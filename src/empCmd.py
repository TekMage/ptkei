    ...
    def HistSend(self, cmd):
        ...
        if cmd[:2] == "!!":
            ...
            if cmd[:1] == "^":
                ...
                if pos == -1:
                    raise IndexError
                cmd = self.hisList[1].replace(cmd[1:pos],
                                     cmd[pos+1:], 1)
    ...
    def __init__(self, ioq, match, disp):
        ...
        if self.commandFormat is not None:
            ...
            if not self.parameterMatch:
                ...
                viewer.Error(r"Invalid parameters.  Form should be:\n%s"
                             % form)
                return
...
class CmdPredict(baseCommand):
    ...
    def receive(self):
        ...
        for i in empSector.sectorPredictions(DB).split(r"\n"):
            self.out.data(i)
    ...
    def checkPending(self, qElem):
        ...
        if qElem.__class__ is self.__class__:
            ...
            for i in qElem.dumpList:
                ...
                if pos != -1 and i not in self.neededDump:
...
class CmdEval(baseCommand):
    ...
    commandFormat = re.compile(
        r"^(?P<type>"+empParse.s_sector+r"|nation|version)\s+"+s_command+r"$")
    def receive(self):
        mm = self.parameterMatch
        type = mm.group('type')
        if type == 'nation' or type == 'version':
            ...
            type = 'SECTOR'
            key = tuple(map(int, [mm.group('sectorX'), mm.group('sectorY')]))
            try:
...
class CmdNova(baseCommand):
    ...
    commandFormat = re.compile(
        "^"+empParse.s_comm+r"\s+"+empParse.s_sector
        +r"\s*(?:"+empParse.s_sector2+r")?\s*$")
    def receive(self):
        mm = self.parameterMatch
        start = list(map(int, [mm.group('sectorX'), mm.group('sectorY')]))
        if mm.group('sector2X'):
            dest = list(map(int, [mm.group('sector2X'), mm.group('sector2Y')]))
        else:
            dest = start
        if dest == start:
        ...
        for newcoord in empDb.sectorNeighbors(dest):
            ...
            if ((own == 0 or own is None or own == CN_UNOWNED)
                and des is not None and des not in r".?s\\"):
...
class CmdLTest(baseCommand):
    ...
    commandFormat = re.compile(r"(?P<landId>\d+)\s+"+
                               r"(?P<xLoc>-?\d+),(?P<yLoc>-?\d+)")
