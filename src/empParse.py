...
def composeHeader(translations, *args):
    ...
    for segment in lastColumns:
        ...
        new.append(r"\n".join(
            [a[startcut:endcut].strip() for a in args]).lstrip())
        startcut = endcut + 1
...
def composeBody(hdr, msg):
    ...
    if len(strs) == len(hdr):
        ...
        new = {}
        for field, value in zip(hdr, strs):
            if field is None:
                continue
            if callable(field):
...
##                 empDb.megaDB['time'].noteTimestamp(self.ctime, float(ts))
                ...
                self.oldownerHack = []
                self.nameHack = []
                for fieldname, pos in zip(self.Hlist,
...
##              print start, end, ' '.join(Dlist[start:end+1])
        ...
        if l == len(self.Hlist):
            ...
            for k, v in zip(self.Hlist, Dlist):
                DDict[k] = v
    ...
    def data(self, msg):
        ...
        if msg_a == "     ":
            ...
            if self.Mpos == 2:
                ...
                msg_b = msg_b.replace("-", "0")
                self.Mhead = [a+b for a, b in zip(self.Mhead, msg_b)]
        else:
            if self.Mpos == 1:
...
class ParseMove(baseDisp):
    ...
    ownSector = re.compile("^Sector "+s_sector+r" is now yours\.$")
#wmf add check for nova and wfind
    ...
    def flush(self, msg, hdl):
        ...
        if mm:
            ...
            for i in self.map[-3:]:
                sects.append(i[3:8])
            coord = list(map(int, [mm.group('sectorX'), mm.group('sectorY')]))
...
class ParseSpy(baseDisp):
    ...
    unitInfo = re.compile(r"^(?:Allied|Enemy|Neutral) \("+s_counName
                          +r"\) unit in "+s_sector+":  "+s_landIdent
                          +r"(?: \("+s_unitStats+r"\))?$")
...
class ParseAttack(baseDisp):
    ...
    attackInfo = re.compile("^"+s_sector
                            ...
                            +r" with approximately "+s_mil+r" military\.$")
...
##   ptkei-0.21 (#  1) bb   battleship (#12) @ 0,-4
    ...
    line = re.compile(r"^\s*"+s_counIdent+" "+s_shipIdent+" @ "+s_sector)
    def data(self, msg):
        self.out.data(msg)
...
class ParseCapital(baseDisp):
    ...
    attach = (('capital', -3),)
    altersDB = ""
    moveCapital = re.compile(r"^Capital now at "+s_sector+r"\.$|"
                             +r"^"+s_sector2+r" is already your capital\.$")
    def data(self, msg):
        self.out.data(msg)
...
class ParseRelations(baseDisp):
    ...
    header = re.compile(r"^\s*"+s_counName
                        +r" Diplomatic Relations Report\t"+s_time+"$")
    line = re.compile(r"^\s*"+s_counId+r"\) "+s_counName+r"\s+"
                      +s_yourRelation+r"\s+"+s_theirRelation+"$")
    def data(self, msg):
        self.out.data(msg)
...
class ParseNation(baseDisp):
    ...
    nationVars = re.compile(
        ...
        "^"+s_counId+" "+s_counName+r" Nation Report\t"+s_time+"$|"
        ...
        r"^No capital\. \(was at "+s_sector2+r"\)$|"
...
class ParseFire(baseDisp):
    ...
    line = re.compile(r"^\s*"+s_shipIdent+" sunk!")
    def data(self, msg):
        self.out.data(msg)
        mm = self.line.match(msg)
        if mm:
...
class ParseShow(baseDisp):
    ...
    def data(self,msg):
        ...
        if 'sector type                 $   lcm  hcm     $      $' in msg:
            self.what = 'sebu'
            return
        if 'sector type            0% 100%   off   def   bility      bonus   pop' in msg:
            self.what = 'sest'
            return
        if 'use1 use2 use3' in msg:
            self.what = 'seca'
            return
        if 'lcm hcm crew' in msg:
            self.what = 'plbu'
            return
        if 'lcm hcm avail' in msg:
            self.what = 'shbu'
            return
        ...
        msg_s = msg.lstrip()
        if msg_s.startswith('sect') and 'maint' in msg_s and 'maxpop' in msg_s and 'bwork' in msg_s:
            self.what = 'e5stat'
            return
        # Empire 5 build format: "sector type  bld mob cost  bld name"
        if msg_s.startswith('sector type') and 'bld' in msg_s and 'bld name' in msg_s:
            self.what = 'e5build'
            return
        if self.what == 'e5stat':
            # "-   (Land             )     0   65535  100"
            ...
            if type not in sectorDesignationConvert \
               or sectorDesignationConvert[type] != name :
                ...
                ParseAttack.attackInfo = re.compile("^"+s_sector
                            +" is a "+s_eff+" "+s_counName+" "+s_sectorName
                            +r" with approximately "+ParseAttack.s_mil+r" military\.$")
                look_info = re.compile(r"^(?:Your|"+s_counIdent+") "+ParseUnits.s_shipOrSector
                                       +" @ "+s_sector+"$")
            return
        if self.what == 'seca':
