...
def sectorPredictions(ldb):
    ...
    if food:
        ...
        if diff > 0:
            s = s + r" NEEDS %d MORE FOOD FOR FULL GROWTH!\n" % diff
    elif popstr:
        s = s + r" Expands to %s.\n" % popstr
    ...
    if civ_limit > 0:
        s = s + r" %d civs not working for production.\n" % civ_limit
    elif civ_limit < 0:
        s = s + r" %d civs needed for max production.\n" % -civ_limit
    if uw_limit > 0:
        s = s + r" %d uws not working for production.\n" % uw_limit
    elif uw_limit < 0:
        s = s + r" %d uws needed for max production.\n" % -uw_limit
