...
class dictDB:
    ...
    def __setstate__(self, state):
        ...
        for valTuple in valueList:
            ...
            pri_key = tuple(pri_val[k] for k in pri_keytype)
        ...
        for dict in self.primary.values():
            for j in dict.keys():
                if j in headers:
                    ...
                    headers[j] = 1
        lst = [list(x) for x in headers.items()]
        for i in lst:
            i.reverse()
        lst.sort()
        lst.reverse()
    ...
    def updates(self, items, returnRemaining=0):
        ...
        self__secondary__items=list(self.secondary.items());self__uDB=self.uDB
        ...
        for dict in items:
            # find the key
            pri_key = tuple(dict[k] for k in self__primary_keytype)

            # list all items being deleted
            dict__items = list(dict.items())
            keys = [x[0] for x in dict__items]
            values = [x[1] for x in dict__items]
            delList = []
            try:
                while 1:
            ...
            try:
                ...
                for sec_type, sec_db in self__secondary__items:
                    sec_key = tuple(d.get(k) for k in sec_type)
    ...
    def __str__(self):
        return "\n".join(map(str, self.primary.items()))
...
class Countries:
    ...
    def getList(self):
        ...
        rng = range(max)
        lst = list(zip(rng, [self.idList.get(x, "") for x in rng]))
        lst.sort()
        return lst
...
def directionToSector(coord, direc):
    ...
    offset = pathOffsets[pathDirections.index(direc)]
    maxx, maxy = megaDB['version']['worldsize']
    halfx = maxx//2
    halfy = maxy//2
...
class DatabaseSaver:
    ...
    def reset(self):
        ...
        megaDB = {
            ...
            'shiptype' : {},
            'landtype' : {},
            'login': {'host':"192.168.40.95", 'port':6665,
    ...
    def load(self, filename):
        ...
        try:
            ...
            if megaDB['DB_Version'] != self.DBVersion:
                raise Exception("PTkEI: Database has an incorrect version number.")
            fl.close()
            self.newDatabase = 0
            self.needSave = 0
            self.resetUpdate()
