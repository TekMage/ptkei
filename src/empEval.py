...
class delayedBinding:
    ...
    for i, j in [
        ...
        #('__cmp__', 'cmp') removed in Python 3
        ]:
...
s_circular = (r"@(?P<cirX>"+empParse.ss_sect+"),(?P<cirY>"+empParse.ss_sect
              +r"):(?P<cirD>\d+)")
sectorsFormat = re.compile(r"^\*$|^"+s_realm+"$|^"+s_sectors
                           +"$|^"+s_circular+"$")
    ...
    while cond:
        ...
        for i in mc.group('var', 'val'):
            if len(i) == 1 and not i in string.digits:
                vars.append(repr(i))
            elif re.compile("^[a-z][a-z0-9_]+$").match(i) and not i in selectors[dbname]:
                vars.append(repr(i))
            else:
                vars.append(i)
...
s_post = r"(?P<post>.*)"
exprFormat = re.compile("^"+s_pre+r"\["+s_eval+r"\]"+s_post+"$")
def estrToExpr(txt):
